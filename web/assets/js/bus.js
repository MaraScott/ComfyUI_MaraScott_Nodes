	...
	registerCustomNodes() {
		class MarasitBusNode {
			constructor() {
				...
				const node = this;
                this.addInput("bus", "BUS")
                this.addOutput("bus", "BUS")
                ...
                this.addOutput("positive", "CONDITIONING")
                this.addInput("negative", "CONDITIONING")
                this.addOutput("negative", "CONDITIONING")
                this.addInput("latent", "LATENT")
				...
				this.addWidget(
					...
					(s, t, u, v, x) => {
						...
						if(this.widgets[0].value !== ''){
							_title = this.widgets[0].value;
						}
						this.title = "Bus Node - " + _title;
						this.update();
				...
				this.onConnectionsChange = function(
					...
					// Handle disconnects to remove data from busData
					if (slotType === 1 && !isChangeConnect) {
